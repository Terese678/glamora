...
;; This is the contract owner address used for platform fee collection and authorization checks
(define-constant CONTRACT-ADDRESS tx-sender)

;; Bridge-adapter contract reference for payment intelligence
(define-constant BRIDGE-ADAPTER .bridge-adapter-v2)
...
(define-constant STORAGE-CONTRACT .storage-v4)
...
(define-data-var storage-contract principal 'STPC6F6C2M7QAXPW66XW4Q0AGXX9HGAX6525RMF8.storage-v4)
...
(define-public (get-nft-listing (token-id uint))
   (ok (contract-call? .storage-v4 get-nft-listing token-id))
)

;; Check if NFT is listed
(define-public (is-nft-listed (token-id uint))
    (ok (is-some (contract-call? .storage-v4 get-nft-listing token-id)))
)
...
(define-public (get-seller-listings (seller principal))
    (ok (contract-call? .storage-v4 get-seller-active-listings seller))
)
...
(define-public (get-total-followers (user principal))
    (match (contract-call? .storage-v4 get-creator-profile user) 
...
(define-public (get-creator-profile (user principal)) 
    (ok (contract-call? .storage-v4 get-creator-profile user))
)
...
(define-public (get-public-user-profile (user principal))
    (ok (contract-call? .storage-v4 get-public-user-profile user))
)
...
(define-public (get-username-owner (username (string-ascii 32)))
    (ok (contract-call? .storage-v4 get-username-owner username))
)
...
(define-public (get-content-details (content-id uint))
    (ok (contract-call? .storage-v4 get-content-details content-id))
)
...
(define-public (get-tip-details (content-id uint) (tipper principal))
    (ok (contract-call? .storage-v4 get-tip-history content-id tipper))
)
...
(define-public (is-user-following (follower principal) (following principal)) 
    (ok (contract-call? .storage-v4 is-following follower following))
)
...
(define-public (get-follow-record (follower principal) (following principal))
    (ok (contract-call? .storage-v4 get-follow-record follower following))
)
...
(define-public (get-user-subscription (user principal))
    (ok (contract-call? .storage-v4 get-user-subscription user))
)
...
(define-public (has-active-subscription (subscriber principal) (creator principal))
    (ok (match (contract-call? .storage-v4 get-user-subscription subscriber)
        subscription-data 
...
(define-public (get-creator-subscription-stats (creator principal))
    (ok (contract-call? .storage-v4 get-creator-subscription-stats creator))
)
...
(define-public (get-nft-metadata (token-id uint))
    (ok (contract-call? .storage-v4 get-nft-metadata token-id))
)

;; Let's look up the details about a fashion collection
(define-public (get-collections-details (collection-id uint))
    (ok (contract-call? .storage-v4 get-collection-data collection-id))
...
(define-public (get-nft-marketplace-stats)
    (let
        (
            ...
            (total-nfts-minted (contract-call? .glamora-nft-v6 get-total-nfts-minted))
            (next-collection-id (contract-call? .glamora-nft-v6 get-next-collection-id))
        )
        (ok {
...
(define-private (has-profile (user principal))
    ...
    (or (is-some (contract-call? .storage-v4 get-creator-profile user))
    (is-some (contract-call? .storage-v4 get-public-user-profile user)))
)
...
(define-private (verify-trusted-contracts)
    (let
        (
            ...
            (storage-hash (unwrap! (contract-hash? .storage-v4) (err u900)))
            
            ;; Get the live fingerprint of glamora-nft-v6
            (nft-hash (unwrap! (contract-hash? .glamora-nft-v6) (err u901)))
        )
...
(define-public (create-creator-profile (username (string-ascii 32)) (display-name (string-utf8 32)) (bio (string-utf8 256)))
    (let
        ...
        (asserts! (is-none (contract-call? .storage-v4 get-username-owner username)) ERR-USERNAME-TAKEN) 

        ;; Check user doesn't already have a profile 
        (asserts! (is-none (contract-call? .storage-v4 get-creator-profile tx-sender)) ERR-PROFILE-EXISTS) 

        ;; SAVE THE DATA - by calling the storage contract
        (unwrap! (contract-call? .storage-v4 create-creator-profile tx-sender username display-name bio) ERR-STORAGE-FAILED)
        ...
        (unwrap! (contract-call? .bridge-adapter-v2 initialize-vault tx-sender) ERR-VAULT-INIT-FAILED)
...
(define-public (create-public-user-profile (username (string-ascii 32)) (display-name (string-utf8 32)) (bio (string-utf8 256)))
    (let
        ...
        (asserts! (is-none (contract-call? .storage-v4 get-public-user-profile tx-sender)) ERR-PROFILE-EXISTS)

        ;; Save the public user profile data by calling the storage contract
        (unwrap! (contract-call? .storage-v4 create-public-user-profile tx-sender username display-name bio) ERR-STORAGE-FAILED)
...
(define-public (update-creator-profile 
    ...
    (begin
        ...
        (asserts! (is-some (contract-call? .storage-v4 get-creator-profile tx-sender)) ERR-PROFILE-NOT-FOUND)
        
        ;; Call storage contract to update the profile
        (unwrap! (contract-call? .storage-v4 update-creator-profile 
                    tx-sender 
                    new-display-name 
                    new-bio) 
            ERR-STORAGE-FAILED)
...
(define-public (update-public-user-profile 
    ...
    (begin
        ...
        (asserts! (is-some (contract-call? .storage-v4 get-public-user-profile tx-sender)) ERR-PROFILE-NOT-FOUND)
        
        ;; Call storage contract to update the profile
        (unwrap! (contract-call? .storage-v4 update-public-user-profile 
                    tx-sender 
                    new-display-name 
                    new-bio) 
            ERR-STORAGE-FAILED)
...
(define-public (publish-content 
    ...
    (let
        ...
        (unwrap! (contract-call? .storage-v4 create-content 
                    content-id
                    tx-sender
                    title
                    description
...
(define-public (tip-creator 
    ...
    (let
        (
            ...
            (content-data (unwrap! (contract-call? .storage-v4 get-content-details content-id) ERR-CONTENT-NOT-FOUND))
        ...
        (if (is-eq payment-token TOKEN-USDCX)
            (unwrap! (contract-call? .bridge-adapter-v2 deposit-to-vault creator creator-amount) ERR-TRANSFER-FAILED)
        ...
        (unwrap! (contract-call? .bridge-adapter-v2 record-payment-source 
                    tx-sender 
                    tip-amount 
                    SOURCE-NATIVE) 
            ERR-TRANSFER-FAILED)
        ...
        (unwrap! (contract-call? .storage-v4 record-tip 
...
(define-public (follow-user (user-to-follow principal))
    (begin
        ...
        (asserts! (not (contract-call? .storage-v4 is-following tx-sender user-to-follow)) ERR-ALREADY-FOLLOWING)

        ;; Create follow relationship
        (unwrap! (contract-call? .storage-v4 create-follow tx-sender user-to-follow) ERR-FOLLOW-FAILED)
...
(define-public (unfollow-user (user-to-unfollow principal)) 
    (begin
        ...
        (asserts! (contract-call? .storage-v4 is-following tx-sender user-to-unfollow) ERR-NOT-FOLLOWING)

        ;; If check passed and you're indeed following them we can proceed to remove the follow relationship
        (unwrap! (contract-call? .storage-v4 remove-follow tx-sender user-to-unfollow) ERR-UNFOLLOW-FAILED)
...
(define-public (subscribe-to-creator (creator principal) (tier uint) (payment-token uint))
    (let
        ...
        (asserts! (is-some (contract-call? .storage-v4 get-creator-profile creator)) ERR-PROFILE-NOT-FOUND)
       ...
       ;; Check the user does not already have active subscription to this creator
        ...
        (if (is-eq payment-token TOKEN-USDCX)
            (unwrap! (contract-call? .bridge-adapter-v2 deposit-to-vault creator creator-amount) ERR-STORAGE-FAILED)
        ...
        (unwrap! (contract-call? .storage-v4 create-subscription
...
(define-public (cancel-subscription (creator principal))
    (let
        (
            ...
            (subscription-data (unwrap! (contract-call? .storage-v4 get-user-subscription tx-sender) ERR-NO-SUBSCRIPTION))
        )
        ...
        (unwrap! (contract-call? .storage-v4 cancel-subscription 
                    tx-sender
                    TIER-BASIC
                    TIER-PREMIUM
                    TIER-VIP
...
(define-public (get-creator-vault-info (creator principal))
    (contract-call? .bridge-adapter-v2 get-earnings-stability-report creator)
)
...
(define-public (get-user-payment-sources (user principal))
    (contract-call? .bridge-adapter-v2 get-payment-source-stats user)
)
...
(define-public (set-vault-threshold (new-threshold uint))
    (contract-call? .bridge-adapter-v2 update-withdrawal-threshold tx-sender new-threshold)
)
...
(define-public (withdraw-from-vault (withdrawal-amount uint))
    (begin
        ...
        (unwrap! (contract-call? .bridge-adapter-v2 complete-vault-withdrawal 
                    tx-sender 
                    withdrawal-amount) 
            ERR-TRANSFER-FAILED)
...
(define-public (create-nft-collection 
    ...
    (begin
        ...
        (unwrap! (contract-call? .glamora-nft-v6 create-fashion-collection 
            collection-name 
            description 
            max-editions) 
            ERR-STORAGE-FAILED)
...
(define-public (list-nft-for-sale (token-id uint) (price uint))
    (begin
        ...
        (asserts! (is-eq (some tx-sender) 
            (unwrap! (contract-call? .glamora-nft-v6 get-owner token-id) ERR-TRANSFER-FAILED)) 
            ERR-UNAUTHORIZED)
        ...
        (unwrap! (contract-call? .storage-v4 create-nft-listing 
            token-id 
            tx-sender 
            price) 
            ERR-STORAGE-FAILED)
...
(define-public (cancel-nft-listing (token-id uint))
    (let
        (
            ...
            (listing-data (unwrap! 
                (contract-call? .storage-v4 get-nft-listing token-id) 
        ...
        (unwrap! (contract-call? .storage-v4 cancel-nft-listing token-id) ERR-STORAGE-FAILED)
...
(define-public (buy-nft (token-id uint))
    (let
        (
            ...
            (royalty-data (unwrap! 
                (unwrap! (contract-call? .storage-v4 get-nft-royalty token-id) ERR-CONTENT-NOT-FOUND)
                ERR-CONTENT-NOT-FOUND))
        ...
        (unwrap! (contract-call? .glamora-nft-v6 transfer
            token-id
            seller
            tx-sender) 
            ERR-TRANSFER-FAILED)
        ...
        (unwrap! (contract-call? .storage-v4 update-royalty-earnings
            token-id
            royalty-amount)
            ERR-STORAGE-FAILED)

        ;; Record the completed sale in storage
        (unwrap! (contract-call? .storage-v4 complete-nft-sale
            token-id
            tx-sender
            sale-price)
            ERR-STORAGE-FAILED)
